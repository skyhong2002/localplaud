...
description: Ask the Codex CLI (GPT-5.6) to run local app verification that needs computer use, browser automation, simulators, screenshots, app launching, or independent runtime inspection. This is how GPT-5.6 is invoked for computer-use work. Use when the user asks Codex to test a flow, verify UI behavior, inspect a running app, capture screenshots, or report confirmation and feedback about implemented behavior that benefits from computer use.
---
...
Do not prompt it like Codex — no elaborate structure or guardrails needed.
