...
- **音檔下載 URL：已確認 ✅（2026-07-10 第二輪 codex，點 Play 觀察）。** 路徑：`GET /file/temp-url/{file_id}`（吃 bare file id，不是 fullname）→ 回傳一個**簽章、會過期的 AWS S3 URL**，指向 `https://apse1-prod-plaud-bucket.s3.amazonaws.com/audiofiles/{file_id}.mp3?AWSAccessKeyId=…&Signature=…&x-amz-security-token=…&Expires=…`。**實際音檔是 .mp3**（list metadata 的 `<id>.opus` fullname 只是命名慣例，別拿來當下載名）。`temp-url` 回傳 JSON 的確切 key 未擷取到（DevTools 無法複製 response），但 client 掃描 JSON 找簽章 URL 即可。
- **逐字稿/摘要的雲端原檔 ✅（bonus）：** `/file/detail/{id}` 的 payload 會解析出一組簽章 S3 資產 URL（host `apse1-prod-plaud-content-storage.s3.amazonaws.com`）：`file_transcript/{id}/trans_result.json.gz`（逐字稿）、`file_summary/{id}/ai_content.md.gz`（摘要 markdown）、`file_outline/{id}/outline.json.gz`。→ 想復用 Plaud 自己的成果（`prefer_cloud_artifacts`）就抓這些、gunzip。摘要是 markdown 可直接用；逐字稿 json 的 schema 未知（見 issue #9）。detail JSON 的完整 key 仍未擷取。
